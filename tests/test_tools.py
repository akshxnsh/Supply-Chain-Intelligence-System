...
def test_get_recent_disruptions_preserves_query_result(monkeypatch):
    rows = [{"id": "event-1", "headline": "Port delay"}]
    monkeypatch.setattr(tools, "query_recent_events", lambda hours: rows)

    assert json.loads(tools.get_recent_disruptions(12)) == rows


def test_generate_purchase_order_preserves_document_contract():
    result = json.loads(
...
def test_save_alert_record_uses_existing_bigquery_layer(monkeypatch):
    captured = {}
    monkeypatch.setattr(tools, "save_alert", lambda alert: captured.update(alert))

    result = tools.save_alert_record(
        alert_id="alert-1",
